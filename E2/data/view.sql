...
SELECT
    ...
    SUM(CASE WHEN b.prim_classe = TRUE THEN b.preco END) AS vendas_1c,
    SUM(CASE WHEN b.prim_classe = FALSE THEN b.preco END) AS vendas_2c

FROM voo v
JOIN aeroporto ap ON v.partida = ap.codigo
JOIN aeroporto ac ON v.chegada = ac.codigo
JOIN bilhete b ON v.id = b.voo_id
